  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (!item.reason || !String(item.reason).trim())
        throw new APIError(
          400,
          `items[${i}]: individual item reason is required`,
        );
    }
    ...
    const requestedVariantIds = items.map(
    ...
    const conflictingReturn = await ReturnRequest.findOne({
      ...
      userId,
      status: {
        $in: ["PENDING", "APPROVED", "PICKUP_SCHEDULED", "REFUND_INITIATED"],
      },
    ...
    const createdReturns = await ReturnRequest.insertMany(returnDocs, {
      session,
    });
    ...
    const affectedSubOrderIds = [...vendorMap.values()].map(
      (v) => v.subOrderId,
    );
    ...
    const subStatusUpdates = affectedSubOrders.map((sub) => {
      ...
      const newSubStatus =
        itemStatuses.length === 1 ? itemStatuses[0] : "MULTI_STATE";
    ...
    const subStatusMap = new Map(
      subStatusUpdates.map((u) => [u.id.toString(), u.status]),
    );
    const finalSubStatuses = allSubOrders.map((s) =>
      subStatusMap.has(s._id?.toString())
        ? subStatusMap.get(s._id.toString())
        : s.status,
    );

    const uniqueMasterStatuses = [...new Set(finalSubStatuses)];
    const newMasterStatus =
      uniqueMasterStatuses.length === 1
