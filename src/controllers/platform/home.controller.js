  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        modules,
        "Active platform modules fetched successfully",
      ),
    );
});
...
export const getHome = catchAsync(async (req, res) => {
  ...
  if (cached) {
    return res
      .status(200)
      .json(new ApiResponse(200, cached, "Home fetched (cached)"));
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, data, "Home fetched successfully"));
});
