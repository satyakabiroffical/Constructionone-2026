  ...
  export const authController = async (req, res, next) => {
            ...
            if (user && user.phoneOtp) {
                ...
                if (user.phoneOtp.lastSentAt) {
                    ...
                    if (timeSinceLastOtp < cooldownPeriod) {
                        ...
                        return res.status(429).json({ success: false, error: `Please wait ${waitTime} seconds before requesting another OTP` });
                    }
                }
                ...
                if (attempts >= MAX_OTP_ATTEMPTS) {
                    ...
                    if (hoursSinceLastAttempt < 24) {
                        return res.status(429).json({ success: false, error: `Daily OTP limit reached. Please try again after 24 hours` });
                    } else {
                        attempts = 0;
                    }
                }
    ...
    export const  verifyOtp=async (req, res)=> {
        try {
            ...
            if (user.disable) {
                return res.status(403).json({ success: false, error: "User is disabled" });
            }

            if (!user.phoneOtp || !user.phoneOtp.codeHash) {
                return res.status(400).json({ success: false, error: "No OTP is pending verification for this user" });
            }
