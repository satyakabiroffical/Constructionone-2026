...
export const getAllCategories = catchAsync(async (req, res) => {
    ...
    if (cached) return res.status(200).json(new ApiResponse(200, cached, 'Categories fetched successfully (cached)'));
...
export const getCategoryById = catchAsync(async (req, res) => {
    ...
    const cached = await RedisCache.get(cacheKey);
    if (cached) return res.status(200).json(new ApiResponse(200, cached, 'Category details fetched successfully (cached)'));
...
export const toggleCategory = catchAsync(async (req, res) => {
    ...
    res.status(200).json(new ApiResponse(200, { isActive: category.isActive }, `Category is now ${category.isActive ? 'active' : 'inactive'}`));
});
