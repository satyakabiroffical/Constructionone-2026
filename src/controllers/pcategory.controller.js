...
export const updatepCategory = async (req, res, next) =>{
    try {
        ...
        if(slug || name){
            slug = await generateSlug(slug || name,
                async (value)=>await Pcategory.exists({
                    slug:value,
                    _id:{ $ne: id }
                })
            )
        ...
        const updated = await Pcategory.findByIdAndUpdate(id,updateData,{new:true,runValidators: true,})
