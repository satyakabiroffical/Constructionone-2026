...
export const UploadSocialMediaPhotos = async (req, res) => {
  try {
    ...
    if (existUrl) return res.status(500).json({ message: "url already exist" });

    if (!url) return res.status(500).json({ message: "please enter url" });

    await Socialmedia.create({
      url,
    });

    res.status(200).json({ message: " url added sucessfully", url });
  } catch (error) {
