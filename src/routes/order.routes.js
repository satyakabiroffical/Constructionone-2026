...
router.route('/order')
    .post(authMiddleware, validateRequest(orderValidation.createOrder), createOrder)
    .get(authMiddleware, getAllOrder)

router.route('/order/:id')
    .get(authMiddleware, getOrder)

router.route('/order/order-status/:id')
    .put(authMiddleware, isAdmin, updateOrder)

 router.route('/order/order-status/:id/cancel')
